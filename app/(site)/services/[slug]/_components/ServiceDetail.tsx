...
interface Props {
  service: ServiceView
  otherServices: RelatedService[]
}

export default function ServiceDetail({ service, otherServices }: Props) {
  return (
    <>
                ...
                <div className="bg-white/[0.04] border border-white/[0.08] rounded-2xl p-5">
                  ...
                  <ul className="space-y-3">
                    ...
                    {service.capabilities.length > 5 && (
                      <li className="text-[0.75rem] text-teal/60 pl-6.5">+{service.capabilities.length - 5} more capabilities</li>
                    )}
                  </ul>
      ...
      <section className="section-padding-sm bg-surface-2 border-t border-surface-border">
        <div className="container-custom">
          ...
          <div className="grid sm:grid-cols-3 gap-5">
            {otherServices.map((s) => (
              <Link key={s.slug} href={`/services/${s.slug}`} className="group block bg-surface border border-surface-border rounded-xl p-5 hover:border-teal/30 hover:shadow-card-hover transition-all duration-250">
      ...
      <section className="section-padding-sm bg-surface border-t border-surface-border">
        <div className="container-custom text-center">
          <h2 className="text-balance mb-4 max-w-[28rem] mx-auto">Ready to get started with {service.shortTitle || service.title}?</h2>
